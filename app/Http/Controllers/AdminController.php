...
class AdminController extends Controller
{
    ...
    function check_login(Request $request){
        ...
        $admin=User::where(['username'=>$request->username,'password'=>sha1($request->password)])->count();
        if($admin>0){
            $adminData=User::where(['username'=>$request->username,'password'=>sha1($request->password)])->get();
